...
if __name__ == "__main__":
    ...
    optimizer = torch.optim.Adam(
        list(v_net.parameters()) + list(a_net.parameters()) + list(f_net.parameters()) + list(p_net.parameters()) + list(
    ...
    for step in range(1, args.num_steps + 1):
        ...
        print(f'Step: {step}, '
              ...
              f'Contrastive_loss: {loss_dict_list_disl["Contrastive_loss"]:.6f}, '  # Update key
              f'LR: {current_lr:.6f} '
              )
        ...
        if test_info["m_ap"]:
            history.update(step, test_info["m_ap"][-1], step, loss_dict_list["U_MIL_loss"],
                           loss_dict_list_disl["MA_loss"], loss_dict_list_disl["M_MIL_loss"],
                           loss_dict_list_disl["Contrastive_loss"], current_lr)  # Update key
            history.save_to_csv('./training_history.csv')
