...
class Dataset(data.Dataset):
    ...
    def _parse_list(self):
        ...
        self.pose_list = list(open(self.pose_list_file, encoding='utf-8'))
    ...
    def __getitem__(self, index):
        ...
        if self.test_mode:
            ...
            f_p = utils.process_feat(f_p, self.max_seqlen, is_random=False)
            if self.return_name == True:
                file_name = self.list[index].strip('\n').split('/')[-1][:-7]
                return f_v, f_a, f_f, f_p, file_name
            return f_v, f_a, f_f, f_p, label
